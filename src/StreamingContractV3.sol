...
import {IStreamingContractV3} from "./interfaces/IStreamingContractV3.sol";

/// @title StreamingContractV3
/// @notice Makes each payment stream a transferable ERC721 claim.
...
contract StreamingContractV3 is IStreamingContractV3, ERC721 {
    ...
    constructor() ERC721("Stream Position", "STREAM") {}
    ...
    function withdrawFromStream(uint256 streamId) external {
        if (streamId >= nextStreamId) revert InvalidStreamId();
        if (msg.sender != ownerOf(streamId)) revert Unauthorized();
        ...
        _transferAsset(stream.tokenAddress, msg.sender, withdrawableAmount);
