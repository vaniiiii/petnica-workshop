...
contract StreamingContractV3 is IStreamingContractV3, ERC721 {
    ...
    /// @notice Overrides ERC721 update to update stream recipient
    ...
    function _update(
        ...
        // Only update existing streams (tokenId < id)
        if (tokenId < id) {
            // Update recipient for transfers/mints, but keep original for burns
            // This prevents withdrawals after burn but preserves stream data
            if (to != address(0)) {
                streams[tokenId].recipient = to;
            }
            // Note: On burn (to == address(0)), we keep the last known recipient
            // This means burned NFTs cannot withdraw, but stream data remains intact
        }
